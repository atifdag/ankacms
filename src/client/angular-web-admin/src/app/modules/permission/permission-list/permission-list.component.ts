...
export class PermissionListComponent implements OnInit {
  ...
  pageSizes: any[];
  userForm: FormGroup;
  listModel = new ListModel<PermissionModel>();
  ...
  list() {
    this.servicePermission.list().subscribe(
      res => {
        if (res.status === 200) {
          this.listModel = res.body as ListModel<PermissionModel>;
        } else {
  ...
  filter() {
    this.adminLayoutComponent.resetCounter();
    this.servicePermission.filter(this.filterModel).subscribe(
      res => {
        if (res.status === 200) {
          this.listModel = res.body as ListModel<PermissionModel>;
        } else {
  ...
  detail(id: string) {
    this.router.navigate(['/Permission/Detail', { id: id }]);
  }
  ...
  onConfirmDelete(id: string) {
    ...
    this.servicePermission.delete(id).subscribe(
      res => {
        if (res.status === 200) {
          this.list();
          this.messageService.add({
  ...
  update(id: string) {
    this.router.navigate(['/Permission/Update', { id: id }]);
  }
