...
export class PermissionAddComponent implements OnInit {
  ...
  model = new AddModel<PermissionModel>();
  ...
  ngOnInit() {
    ...
    this.servicePermission.beforeAdd().subscribe(
      res => {
        if (res.status === 200) {
          this.model = res.body as AddModel<PermissionModel>;
  ...
  onSubmit() {
    ...
    this.servicePermission.add(this.model).subscribe(
      res => {
        if (res.status === 200) {
