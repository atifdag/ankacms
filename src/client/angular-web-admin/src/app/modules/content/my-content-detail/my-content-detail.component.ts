...
@Component({
  selector: 'app-my-content-detail',
  templateUrl: './my-content-detail.component.html',
  styleUrls: ['./my-content-detail.component.css']
})
export class MyContentDetailComponent implements OnInit {
  loading = true;
  ...
  ngOnInit() {
    ...
    this.serviceContent.myContentDetail(this.contentId, this.languageId).subscribe(
      responseContent => {
        this.loading = false;
        if (responseContent.status === 200) {
          this.model = responseContent.body as DetailModel<ContentModel>;
  ...
  updateClick() {
    this.router.navigate(['/Content/MyContentUpdate', { contentId: this.contentId, languageId: this.languageId }]);
  }

  onConfirmDelete() {
    this.messageService.clear('d');
    this.serviceContent.myContentDelete(this.contentId, this.languageId).subscribe(
      res => {
        if (res.status === 200) {
          this.messageService.add({
            severity: 'success',
