...
export class ContentListComponent implements OnInit {
  ...
  list() {
    ...
    this.serviceContent.list().subscribe(
  ...
  filter() {
    this.adminLayoutComponent.resetCounter();
    this.serviceContent.filter(this.filterModel).subscribe(
      res => {
        if (res.status === 200) {
          this.listModel = res.body as ListModel<ContentModel>;
        } else {
  ...
  detail(contentId: string, languageId: string) {
    this.router.navigate(['/Content/Detail', { contentId: contentId, languageId: languageId }]);
  }
  ...
  onConfirmDelete(data: any) {
    ...
    this.serviceContent.delete(data.contentId, data.languageId).subscribe(
      res => {
        if (res.status === 200) {
          this.list();
          this.messageService.add({
  ...
  update(contentId: string, languageId: string) {
    this.router.navigate(['/Content/Update', { contentId: contentId, languageId: languageId }]);
  }
