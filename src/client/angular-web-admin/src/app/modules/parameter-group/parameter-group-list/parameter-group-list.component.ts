...
export class ParameterGroupListComponent implements OnInit {
  ...
  pageSizes: any[];
  userForm: FormGroup;
  listModel = new ListModel<ParameterGroupModel>();
  ...
  list() {
    this.serviceParameterGroup.list().subscribe(
      res => {
        if (res.status === 200) {
          this.listModel = res.body as ListModel<ParameterGroupModel>;
        } else {
  ...
  filter() {
    this.adminLayoutComponent.resetCounter();
    this.serviceParameterGroup.filter(this.filterModel).subscribe(
      res => {
        if (res.status === 200) {
          this.listModel = res.body as ListModel<ParameterGroupModel>;
        } else {
  ...
  detail(id: string) {
    this.router.navigate(['/ParameterGroup/Detail', { id: id }]);
  }
  ...
  onConfirmDelete(id: string) {
    ...
    this.serviceParameterGroup.delete(id).subscribe(
      res => {
        if (res.status === 200) {
          this.list();
          this.messageService.add({
  ...
  update(id: string) {
    this.router.navigate(['/ParameterGroup/Update', { id: id }]);
  }

  newRecordClick(event: Event) {
    this.router.navigate(['/ParameterGroup/Add']);
  }
}
