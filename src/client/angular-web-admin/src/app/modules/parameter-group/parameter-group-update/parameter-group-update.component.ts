...
export class ParameterGroupUpdateComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.route.paramMap.subscribe(params => {
      ...
      this.serviceParameterGroup.beforeUpdate(this.id).subscribe(
        res => {
          this.loading = false;
          if (res.status === 200) {
            this.model = res.body as UpdateModel<ParameterGroupModel>;
  ...
  onSubmit() {
    ...
    this.serviceParameterGroup.update(this.model).subscribe(
      res => {
        if (res.status === 200) {
