...
export class LanguageListComponent implements OnInit {
  ...
  pageSizes: any[];
  userForm: FormGroup;
  listModel = new ListModel<LanguageModel>();
  ...
  list() {
    this.serviceLanguage.list().subscribe(
      res => {
        if (res.status === 200) {
          this.listModel = res.body as ListModel<LanguageModel>;
        } else {
  ...
  filter() {
    this.adminLayoutComponent.resetCounter();
    this.serviceLanguage.filter(this.filterModel).subscribe(
      res => {
        if (res.status === 200) {
          this.listModel = res.body as ListModel<LanguageModel>;
        } else {
  ...
  detail(id: string) {
    this.router.navigate(['/Language/Detail', { id: id }]);
  } 
  ...
  onConfirmDelete(id: string) {
    ...
    this.serviceLanguage.delete(id).subscribe(
      res => {
        if (res.status === 200) {
          this.list();
          this.messageService.add({
  ...
  update(id: string) {
    this.router.navigate(['/Language/Update', { id: id }]);
  }
