...
export class LanguageDetailComponent implements OnInit {
  ...
  model = new DetailModel<LanguageModel>();
  ...
  ngOnInit() {
    ...
    this.route.paramMap.subscribe(params => {
      ...
      this.serviceLanguage.detail(this.id).subscribe(
        res => {
          this.loading = false;
          if (res.status === 200) {
            this.model = res.body as DetailModel<LanguageModel>;
  ...
  updateClick() {
    this.router.navigate(['/Language/Update', { id: this.id }]);
  }

  onConfirmDelete() {
    this.messageService.clear('d');
    this.serviceLanguage.delete(this.id).subscribe(
      res => {
        if (res.status === 200) {
          this.messageService.add({
            severity: 'success',
